...
```bash
cd ~/kernel
wget -c https://mirrors.edge.kernel.org/pub/linux/kernel/projects/rt/6.10/patch-6.10-rc5-rt9.patch.xz
xz -d patch-6.10-rc5-rt9.patch.xz
```
## Go back into the cloned linux
```bash
cd ~/linux
```
## Undo prior patch, if necessary, in this case the one for 6.10-rc4-rt7 – please perform this only, if you've applied a patch before and the respective kernel patch is still available in ~/kernel!
```bash
#patch -R -p1 < ~/kernel/patch-6.10-rc4-rt7.patch
```
...
## Patch the kernel
```bash
patch -p1 < ~/kernel/patch-6.10-rc5-rt9.patch
```
